...
import { markAtomicMembers } from "./baseNumber.js"
  ...
  return new Promise((resolve, reject) => {
    ...
    if (config.timeoutLong) {
      timeoutLong = setTimeout(() => {
        config.info(`Command "${cmd}" taking more than ${config.timeoutLong / 1000} seconds.`)
      }, config.timeoutLong)
    }
    // Kill stale commands
    if (config.timeoutStale) {
      timeoutStale = setTimeout(() => {
        ...
        config.info(`Command "${cmd}" took more than ${config.timeoutStale / 1000} seconds, killing it.`, config.backend.hosts.length > 1 ? ` Also rotating backend host to ${config.backend.hosts[backendIndex]}:${config.backend.ports[backendIndex]}.` : "")
        terminate(pid)
      }, config.timeoutStale)
    }
  })
      ...
      try {
        ...
        if (error.message.includes("Connection refused") && config.backend.hosts.length > 1) {
          if (backendIndexBefore === backendIndex) {
            backendIndex = (backendIndex + 1) % config.backend.hosts.length
            config.info(`Connection error when analyzing ${notation}, rotating host to ${config.backend.hosts[backendIndex]}:${config.backend.ports[backendIndex]}.`)
          } else {
            config.info(`Connection error when analyzing ${notation}, trying again with already rotated host.`)
          }
          continue
        }
