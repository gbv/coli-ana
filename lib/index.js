...
export async function findMembers({ member: search, limit, offset }) {
  ...
  try {
    ...
    result = await prisma.$queryRaw(sql + ` limit ${limit} offset ${offset}`)
    // Also find total count for pagination
    const totalCount = await prisma.$queryRaw(sql.replace("select *", "select count(*)"))
