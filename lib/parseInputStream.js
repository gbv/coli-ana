 ...
 * ```
 *
 * @param {stream} input input stream of vc_day slim data
    ...
    if (startMatch) {
      ...
      if (member) {
        ...
        if (current.notation[0].length !== member.notation[1].length) {
          options.warn && options.warn(`Error: Second notations should have the same length as analyzed notation, skipping. (${current.notation[0]})`)
        ...
        options.warn && options.warn(`Warning: Could not convert DDC notation ${lineMatch[3]}`)
      }
    } else {
      options.warn && options.warn(`Warning: Could not parse line ${line}`)
    }
  }
}
