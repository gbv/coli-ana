  ...
  for await (const line of rl) {
    ...
    if (startMatch) {
      ...
      if (member) {
        ...
        config.warn(`Warning: Could not convert DDC notation ${notation}`)
      }
    } else {
      config.warn(`Warning: Could not parse line ${line}`)
    }
  }
  if (current) {
    currentHasError = currentHasError || !checkConcept(current)
...
function checkConcept(concept) {
  ...
  if (!memberList.length) {
    config.warn(`Analysis of ${notation} is empty`)
    return
  }

  if (!memberList.every(m => m.notation[1].length === notation[0].length)) {
    config.warn(`Notation length mismatch in ${notation}`)
    return
  }
  ...
  for(let i=0; i<memberList.length; i++) {
    ...
    count[id] = count[id] || 0
    if (count[id]++ >= 2) {
      config.warn(`Error: Unexpected duplicate ${id} in analysis of ${notation})`)
      return
    }
  }
